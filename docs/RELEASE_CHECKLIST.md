...
- Review success-path visibility is checked with a completed-case QA account or an approved test session.
- Accounts without completed cases show the server's completion-required review policy instead of creating a review.
...
- With a completed-case QA account or approved test session, create an integer review and confirm detail-screen visibility after append or reload.
- With a QA account that has no completed case, confirm the review submit path shows the server completion-required policy.
