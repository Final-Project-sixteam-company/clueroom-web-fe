...
## ✅ Phase 6 — Splash + Onboarding 이식 완료 (2026-06-20, 브랜치 `feat/react-migration-tokens`)
koo #5 의 마지막 두 화면을 픽셀+모션 복원. 정본 = `lib/screens/splash_screen.dart` · `onboarding_screen.dart`.
- **SplashScreen**(`src/components/screens/SplashScreen.{tsx,module.css}`): 라디얼 배경(center 50% 40%, tealBase .06→roseBase .04→ink950) + 로고 배지 80(login orb 와 동일 + **SVG 십자선** 4선 CustomPaint 이식) + 'ClueRoom'(hero28, ls−0.5) + 'AI' 배지(primarySoft·r1·mono10) + 펄스 라벨('INVESTIGATING · 조사 준비 중', opacity .3↔1 900ms ease-in-out 무한) + 하단 버전('A DETECTIVE GAME · v0.9.2', sp8). 진입 = 콘텐츠 페이드+슬라이드업(translateY 6%→0, **1100ms easeOut**, 배경 제외). 비-탭 전체화면(셸 bare).
- **OnboardingScreen**(`src/components/screens/OnboardingScreen.{tsx,module.css}`): 5슬라이드 — 건너뛰기(ghost 우상단) + 슬라이드 트랙(Flutter `PageView` → **가로 scroll-snap**, 네이티브 스와이프 + '다음'은 다음 페이지 smooth scroll, onScroll 로 활성 페이지 추적) + 인디케이터(활성 width 20/비활성 6, AnimatedContainer dur2) + 버튼(마지막 '시작하기' loading=navigating, 그 외 '다음'). 슬라이드 = 아이콘 배지 80(teal→sky 135° 그라디언트 r6, 아이콘 36 ink950) + titleL + body(textSub, lh 1.65). 상하 Spacer flex 2:3 비율 보존. 아이콘 Material→lucide(Search/FileText/Users/Lightbulb/Star).
- **부트 라우팅**(App): 초기 view `home`→**`splash`**. `useSplashGate` 대신 App 인라인(orchestrator): 2.2s 타이머(`SPLASH_DURATION_MS`) + authReady 대기 → 분기. 온보딩 플래그 = `src/onboarding/onboardingStorage.ts`(`hasSeenOnboarding`/`markOnboardingSeen`, localStorage `clueroom.onboardingComplete` ← Flutter SharedPreferences 'onboarding_complete' 대응). **미시청→onboarding, 시청→home**. 온보딩 완료/건너뛰기 → `markOnboardingSeen()` + home. 기존 `if (!authReady)` 부트 게이트(StateBlock) 제거 — 스플래시가 흡수.
- **⚠ koo 확인(의도적 선택, 한 줄로 뒤집기 쉬움)**: ① **분기 목적지 = home(웹 동작 보존, koo #4)** — Flutter 는 미로그인 시 `LoginScreen` 강제지만 웹 home 은 미로그인도 열려 있음(HomeScreen isLoggedIn + 로그인 CTA). login 강제로 바꾸려면 분기 한 줄 수정. ② **스플래시 매 부트 2.2s 노출**(Flutter 동작 그대로) — '첫 실행만'으로 바꾸려면 플래그 게이트 추가. ③ 스와이프 = scroll-snap(PageView 대응) — 드래그 모션은 네이티브 스크롤에 위임. ④ 라디얼 radius .9 는 CSS 근사(매우 옅은 6%/4% alpha라 시각차 미미).
