...
## ▶ 다음 작업 (둘 중 koo 지정)
A. **game_modals 바텀시트 패스** — 위 4종 바텀시트 포팅(드래그 핸들·시트 모션). 도메인 컴포넌트의 마지막 조각. (디자인/픽셀 성격)
...
> 킷·도메인 컴포넌트를 화면에 실제로 올리는 **Phase 5 화면 정합**은 위 둘과 인터리브 가능. 브라우저 육안 확인이 필요하면 킷/도메인 갤러리(임시 view)를 먼저 깔 수도 있음(프리미티브 12종 + 도메인 카드 한눈에).
