...
### ⏭ 컴포넌트 킷에서 아직 안 한 것 (도메인 컴포넌트 패스)
계획 §6 하단 "도메인 컴포넌트"는 별도 패스로 남음 — 프리미티브 킷 위에 기능별로 올림:
- `AssetImage`/`CharacterPortrait`/`EvidenceThumb`/`ScenarioCoverImage` (asset_image_widget.dart 등)
- `EvidenceTile`(주력) vs `EvidenceItem`(suspect 상세 전용) — 역할 구분, 둘 다 필요. `EvidenceCategoryBadge`.
- `SuspectCard`, `TimelineList`, `ImageViewerModal`(image_viewer_modal 정본 + image_viewer 레거시 통합)
- game_modals 시트: `HintSheet`/`EvidencePresentSheet`/`CaseBriefingSheet`/`ReviewWriteSheet`

## ▶ 다음 작업 (둘 중 koo 지정)
A. **도메인 컴포넌트 패스** — 위 §"아직 안 한 것" 목록을 프리미티브 킷 위에 포팅. game_modals.dart·suspect_card.dart·timeline_list.dart·evidence_*.dart·asset_image_widget.dart·review_write_sheet.dart 정본. (디자인/픽셀 성격, 킷의 연장)
B. **Phase 4 — 기능 훅 분해** — `useScenarios`/`useGameSession`/`useRecords`/`useResult`로 god-state 마저 분리, `App.tsx`는 `view` 스위치만. ⚠ **회귀 금지 나머지 2종**(30초 status-gated 폴링·1초 타이머 `loadCaseRef`)은 `useGameSession` 추출 시 다룸 — **추출 후 단위테스트**(refreshController/withAuthRetry와 동일 "순수 추출 → node:test" 레시피).

> 킷·도메인 컴포넌트를 화면에 실제로 올리는 **Phase 5 화면 정합**은 위 둘과 인터리브 가능. 브라우저 육안 확인이 필요하면 킷 갤러리(임시 view)를 먼저 깔 수도 있음.
