...
| 제외 범위 | 외부 미니앱 패키징, miniapp-specific login, Android APK/AAB, FCM |
...
| OAuth smoke | 운영 웹에서 Google/Kakao 버튼·설정 smoke 확인. 실제 provider login은 배포 후 gate로 재확인 |
...
| AI quota UX | `aiQuota.stage` 기반 안내 배너 구현. non-`NONE` threshold 응답은 QA 조건에서 확인 |
| 운영 smoke | library/detail/mobile CTA/case tabs/interrogation/final validation, bookmark persistence core flow PASS |
| 릴리스 게이트 | provider OAuth login, review success path, refresh-cookie retry를 배포 후 재확인 |

관련 운영 문서:

| 문서 | 역할 |
|---|---|
| [docs/PORTING_STATUS.md](docs/PORTING_STATUS.md) | Flutter Android 흐름을 React/Vite 웹으로 이식한 현재 구현 상태 |
| [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) | 운영 배포 전/후 build, env, OAuth, smoke gate |
...
- Google 또는 Kakao 로그인
- 기본 공개 루트(`/`)에서 QA/dev login UI가 보이지 않는지 확인
- 브라우저 localStorage에 refresh token이 저장되지 않는지 확인
- 시나리오 목록과 상세 진입
- 플레이 세션 시작 또는 복구
- 심문 질문 1회 전송
- 백엔드가 `aiQuota.stage != "NONE"`을 반환하는 QA/threshold 조건에서는 quota 안내가 표시되는지 확인
- 북마크 저장/해제 후 저장한 사건 화면과 상세 버튼 상태가 유지되는지 확인
- 리뷰는 완료된 사건이 있는 QA 계정/승인 세션에서 1~5 정수 별점 작성 후 상세 화면 반영을 확인
- 완료 이력이 없는 QA 계정에서는 서버의 “플레이 완료 후 리뷰 가능” 차단 정책이 표시되는지 확인
...
- 외부 미니앱 패키징과 miniapp-specific login은 폐기되었습니다.
